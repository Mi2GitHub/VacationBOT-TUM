...
export default function AdminPage() {
  ...
  const submitHandler = (e) => {
    ...
    if (!messageBody.trim()) {
      ...
      setMessages([
        ...messages,
        { body: messageBody, from: "VacationBOT", to: selectedUser.name }, //Ändert Name bei AdminPage
      ]);
      setTimeout(() => {
        socket.emit("onMessage", {
          body: messageBody,
          from: "VacationBOT", //change, so bekommt der User keine Nachrichten mehr.
  ...
  return (
    <Row>
      <Col sm={3}>
        {users.filter((x) => x.name !== "VacationBOT").length === 0 && (
          <Alert variant="info">No User Found</Alert>
        )}
        <ListGroup>
          {users
            .filter((x) => x.name !== "VacationBOT")
            .map((user) => (
              <ListGroup.Item
                action
                key={user.name}
